          ...
          try {
            ...
            if (data.success) {
              ...
              const mobileCard = document.getElementById(
                `holding-mobile-${portfolioId}`,
              );
              const invest = parseFloat(row.dataset.invest);
              const current = parseFloat(row.dataset.current);
              const pl = parseFloat(row.dataset.pl);

              let totalInvest = parseFloat(
                document.getElementById("total-invest").dataset.value,
              );
              let totalCurrent = parseFloat(
                document.getElementById("total-current").dataset.value,
              );
              let totalPL = parseFloat(
                document.getElementById("total-pl").dataset.value,
              );
              ...
              if (totalInvest != 0) {
                totalPercent =
                  ((totalCurrent - totalInvest) * 100) / totalInvest;
              }
  ...
  function recalculatePortfolio() {
    ...
    rows.forEach((row) => {
      ...
      const qty = qtyInput
        ...
        : parseFloat(qtyCell.innerText);

      const purchase_price = parseFloat(
        row.querySelector(".purchase_price").innerText.replace("$", ""),
      );
      const current_price = parseFloat(
        row.querySelector(".current_price").innerText.replace("$", ""),
      );
